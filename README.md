...
**Error:** `Missing RESEND_API_KEY - Required for email sending`
...
**Error:** `Invalid EMAIL_SENDER_ADDRESS format`

**Solution:**

- Ensure the email address is a valid format (e.g., `noreply@yourdomain.com`)
...
1. Check if Docker is running: `yarn docker:up`
2. Verify PostgreSQL is running: `docker ps`
3. Check `DATABASE_URL` in `.env` matches Docker configuration
4. Default connection: `postgresql://authly:authly_dev_password@localhost:5433/authly`
...
2. Check `EMAIL_SENDER_ADDRESS` uses a verified domain
...
2. Verify OAuth credentials are correct in Google Cloud Console
3. Check redirect URIs match your application URL

**Error:** `Redirect URI mismatch`

**Solution:**

1. Add your callback URL to Google OAuth settings
2. Format: `https://yourdomain.com/api/auth/callback/google`
3. For local development: `http://localhost:5173/api/auth/callback/google`
...
- For development, adjust rate limits in `middleware.ts`

**Note:** Rate limits are per-IP and reset automatically after the window expires.
